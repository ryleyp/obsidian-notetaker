...
` : ""}${exampleRows?.length ? `
REAL ROWS THE CSM HAS ALREADY REVIEWED AND FILED IN SALESFORCE — match their title style, classification judgment, and comment voice exactly:
${exampleRows.map((r) => JSON.stringify({ title: r.title, type: r.type, subtype: r.subtype, comments: r.comments })).join("\n")}
` : ""}
...
POSTABILITY CHECK — every row is pasted into Salesforce as written. Before emitting a row confirm: 800 characters / 120 words or fewer; no "I", "we", "our", "my"; the CSM's name appears nowhere (say "CSM"); no source markers like [T1], no Markdown, no placeholders like [Name] or [#] (an honest "TBD" for an attendee count is fine); no "CSM attended / observed / listened"; no corporate filler; no other row already cites this source note; all three labels present (Summary, Outcomes, Next steps); a Summary that says what the CSM did rather than only that a meeting happened; no revenue causation the sources do not state; Type and Subtype copied character-for-character. A row that fails any of these is rewritten, not emitted.
...
export async function POST(request) {
  try {
    ...
    // Filed rows come from the client with real names — sanitize like note content.
    const sanitizeField = (value) => applyReplacements(applyCorrections(String(value || ""), corrections), replacements);
